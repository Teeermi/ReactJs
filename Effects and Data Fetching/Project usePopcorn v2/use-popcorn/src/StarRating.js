...
export default function StarRating({ maxRating = 5, color = "#fcc419", size = 24, className = "", messeges = [], defaultRating = 0, setMovieRating }) {
      ...
      <div style={starContainerStyle}>
        {Array.from({ length: maxRating }, (_, i) => (
          <Star
            ...
            onMouseEnter={() => setTepmRating(i + 1)}
            onMouseLeave={() => setTepmRating(0)}
            color={color}
            size={size}
          />
        ))}
...
function Star({ onClick, full, onMouseEnter, onMouseLeave, color, size }) {
  ...
  return (
    <span role="button" style={starStyle} onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      {full ? (
